...
public class NewRefocusActivity extends Activity {
	...
	private final static String TAG = "NewRefocusActivity";
	private final static int NORMAL_REFOCUS_MODE = 0x01;	//拍照时使用
	...
	private ImageRefocus mImageRefocus;	//算法引擎
	private Bitmap mBmp;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_new_refocus);
	...
	public void onClickSelectCalibration(View srcView){
		...
		startActivityForResult(intent, NewRefocusActivity.REQUEST_CODE_OPENDIALOG_CALIBRATION);
	...
	public void onClickSelectMainFile(View srcView){
		...
		intent.putExtra("DefaultFilePath", strDefaultWorkDir == null?Environment.getExternalStorageDirectory().getPath():strDefaultWorkDir);  
		intent.putExtra("DefaultFileName", "default.lag");
		startActivityForResult(intent, NewRefocusActivity.REQUEST_CODE_OPENDIALOG_MAIN);
	...
	public void onClickSelectAuxFile(View srcView){
		...
		intent.putExtra("DefaultFilePath", strDefaultWorkDir == null?Environment.getExternalStorageDirectory().getPath():strDefaultWorkDir);  
		intent.putExtra("DefaultFileName", "default.lag");
		startActivityForResult(intent, NewRefocusActivity.REQUEST_CODE_OPENDIALOG_AUX);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		switch (requestCode) {
		   //标定文件 
		   case NewRefocusActivity.REQUEST_CODE_OPENDIALOG_CALIBRATION:
		    ...
		    break;
			   //主摄文件
			   case NewRefocusActivity.REQUEST_CODE_OPENDIALOG_MAIN:
				   if (resultCode == RESULT_OK){
					   Bundle b = data.getExtras();
					   String[] fileList = b.getStringArray("FilePathName");
					   if(fileList.length == 1){
						   ...
						   mMainFileTV.setText(getFileName(fileList[0]));
							...
							if(wh[0] == 0 || wh[1] == 0){
								Toast.makeText(NewRefocusActivity.this, "file name error", Toast.LENGTH_SHORT).show();
							...
							if(fileList[0].indexOf("nv21")!=-1 || fileList[0].indexOf("NV21")!=-1){
								...
								Toast.makeText(NewRefocusActivity.this, "file surfix error", Toast.LENGTH_SHORT).show();
							}
					   }
			   ...
			   break;
			   //副摄文件 
			   case NewRefocusActivity.REQUEST_CODE_OPENDIALOG_AUX:
				   if (resultCode == RESULT_OK){
					   Bundle b = data.getExtras();
					   String[] fileList = b.getStringArray("FilePathName");
					   if(fileList.length == 1){
	...
	public void onClickOK(View srcView){
		...
		//放在此处，主要是通过加载标定数据的log，查看标定的主副摄sensor原图宽和高
		if(bCalibrationData == null){
			   Toast.makeText(NewRefocusActivity.this, "calibration error", Toast.LENGTH_SHORT).show();
		...
		mImageRefocus.Init(NORMAL_REFOCUS_MODE);
		//加载标定，读取主副摄标定尺寸。
		...
		if(nMainCropWidth == 0 || nMainCropHeight == 0 || nAuxCropWidth == 0 || nAuxCropHeight == 0){
			...
			Toast.makeText(NewRefocusActivity.this, "crop size error", Toast.LENGTH_SHORT).show();
		...
		if(bMainFileData == null || bAuxFileData == null){
			...
			Toast.makeText(NewRefocusActivity.this, "input image error", Toast.LENGTH_SHORT).show();
		...
		mImageRefocus.SetDCIRParam(0, 70);
		mImageRefocus.SetParam(nFocusX, nFocusY, 50);
		byte[] resultData = mImageRefocus.GetImageResult(bMainFileData, bMainFileData.length, bAuxFileData, bAuxFileData.length, nMainWidth, nMainHeight, nAuxWidth, nAuxHeight);
		if(resultData != null) {
			int pointIndex = strMainFile.indexOf('.');
			mBmp = YuvUtil.nv21ToBitmap(resultData, nMainWidth, nMainHeight);
			String fileSufix = strMainFile.substring(pointIndex); //后缀名包含. .nv21
			String out_file = strMainFile.substring(0, pointIndex) + "_bokeh" + fileSufix;
			FileUtil.saveImage(out_file, resultData);
			Toast.makeText(NewRefocusActivity.this, "Succeed", Toast.LENGTH_SHORT).show();
		}
